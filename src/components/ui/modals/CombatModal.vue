    ...
    <div class="combatModal">
        ...
        <div class="combatFrame" v-if="!shipsSelected && checkForUnits()">
            ...
            <div v-for="(ship, index) in ships" class="combatUnitFrame" :key="ship.key">
                <img :src="require('../../../assets/ui-items/' + ship.unit.unitName + '.png')" width="49px" height="42px" />
            ...
            <button class="combatButton" v-if="!shipsSelected" @click="openUnitsFrame">Select Units</button>
        </div>
        ...
        <div class="combatFrame" v-if="shipsSelected">
            ...
            <div class="combatUnitScrollContainer scrollerFirefox">
                <div v-for="(unit, index) in units" class="combatUnitFrame" :key="unit.key">
                    <img :src="require('../../../assets/ui-items/' + unit.unit.unitName + '.png')" width="49px" height="42px" />
                    <div class="combatUnitsInStore">
                        <p>{{ unit.amount }}</p>
                    </div>
...
export default {
    ...
    methods: {
        ...
        addShips: function (shipType, shipsCarryingCapacity, index) {
            ...
            this.shipsDictionaryList.push({ shipType: shipType, shipCarryingCapacity: shipsCarryingCapacity, shipsAmount: this.inputValues[index] });
        },
        totalCarryingCapacity: function () {
            let totalCarryingCapacity = 0;
            for (let i = 0; i < this.shipsDictionaryList.length; i++) {
                totalCarryingCapacity += this.shipsDictionaryList[i].shipCarryingCapacity * this.shipsDictionaryList[i].shipsAmount;
            }
        ...
        addShipsToUnitList: function () {
            for (let i = 0; i < this.shipsDictionaryList.length; i++) {
                let shipUnit = { unitType: this.shipsDictionaryList[i].shipType, amount: this.shipsDictionaryList[i].shipsAmount };
        ...
        checkForUnits: function () {
            if (this.unitsInVillage.length === 0) {
                ...
                for (i; i < this.unitsInVillage.length; i++) {
                    if (this.unitsInVillage[i].unit.unitName === 'TransportShip' || this.unitsInVillage[i].unit.unitName === 'CombatShip') {
                        shipsInvillage = true;
                    }
                }
