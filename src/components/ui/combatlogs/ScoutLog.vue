    ...
    <div v-if="item">
        ...
        <div class="afterFightData" v-if="userWon && isTheAttacker">
            ...
            <span class="pillagedResources" v-if="item.attackLog.pillagedResources">
                <span class="resource" v-for="(amount, resource) in item.attackLog.pillagedResources" :key="resource.id">
                    <img style="margin-right: 4px" :src="require('../../../assets/ui-items/' + resource + '.png')" width="21px" height="17px" />
                    <span>{{ amount }}</span>
                </span>
            </span>
...
export default {
    ...
    computed: {
        ...
        userWon() {
            return this.isTheAttacker ? this.item.attackLog.attackerWon : !this.item.attackLog.attackerWon;
        },
    },
    methods: {
        selectLog() {
