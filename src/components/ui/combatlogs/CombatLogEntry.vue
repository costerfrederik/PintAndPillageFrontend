<template>
    <div v-if="item" @click="selectLog" class="combatLogEntry">
        <p v-if="isTheAttacker">You {{ item.attackLog.isScoutAttack ? 'scouted' : 'attacked' }} {{ item.defendingUsername }}</p>
        <p v-else>{{ item.attackingUsername }} {{ item.attackLog.isScoutAttack ? 'scouted' : 'attacked' }} you</p>
