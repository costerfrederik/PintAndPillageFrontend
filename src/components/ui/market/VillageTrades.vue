    ...
    <div class="outerVillageTradeContainer scrollerFirefox" v-if="hasMultipleVillages()">
        <div style="display: flex; flex-direction: row; justify-content: center; height: 140px">
            <div style="margin-top: 15px">
                <div class="villageSelectorMarket" @click="isDroppedDown = !isDroppedDown" v-click-outside="closeDropdown">
            ...
            <resource-selector v-if="resources" :resources="Object.keys(resources)" @selectedUpdate="updateTradeResource($event)"></resource-selector>
        </div>
        <button @click="setOffer()" class="tradeButton">Transfer Goods</button>
    </div>
    <div v-else style="display: flex; justify-content: center; align-items: center; color: white; width: 700px">
        <h2 style="width: 70%; text-align: center">
            You currently don't have multiple villages to trade with. Train a Jarl and Settle a new piece of land!
        </h2>
    </div>
</template>
...
export default {
    ...
    computed: {
        ...
        villageList: function () {
            let unFilteredList = this.$store.getters.villageList;
            let filteredList = [];
            ...
            this.$store
                ...
                .catch((err) => {
                    if (err.response.status === 412) {
                        this.$toaster.error('Preconditions not met, market in receiving village must be level 5 or higher');
                    } else {
